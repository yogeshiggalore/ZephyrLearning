/*
 * Project kt_03: blink two leds, one led with one hardware timer with 730ms and another with 456ms
...
#define LED_RED_TIMER_VALUE_US		730000
#define LED_APP_TIMER_VALUE_US		456000
...
struct counter_alarm_cfg timer_1_cfg;
struct counter_alarm_cfg timer_2_cfg;

const struct device *const dev_timer_1 = DEVICE_DT_GET(TIMER1);
const struct device *const dev_timer_2 = DEVICE_DT_GET(TIMER2);

/* timer1 callback function */
void timer_1_callback_fun(const struct device *dev, uint8_t chan_id, uint32_t ticks, void *user_data);
...
int main(void)
{
	...
	if (!device_is_ready(dev_timer_1))
	{
		return 0;
	}

	counter_start(dev_timer_1);
	

	timer_1_cfg.flags = 0;
	timer_1_cfg.ticks = counter_us_to_ticks(dev_timer_1, LED_RED_TIMER_VALUE_US);
	timer_1_cfg.callback = timer_1_callback_fun;
	timer_1_cfg.user_data = &timer_1_cfg;

	err = counter_set_channel_alarm(dev_timer_1, ALARM_CHANNEL_ID, &timer_1_cfg);

	counter_start(dev_timer_2);

	timer_2_cfg.flags = 0;
	timer_2_cfg.ticks = counter_us_to_ticks(dev_timer_2, LED_APP_TIMER_VALUE_US);
