/*
 * Project kt_04: blink two leds, one led with with 730ms and another with 456ms using single timer
...
#define TIMER_PERIOD_VALUE      1
#define TIMER_DURATION_VALUE    1
...
int main(void)
{
	...
	/* start timer */
	k_timer_start( &led_timer, K_MSEC( TIMER_DURATION_VALUE ), K_MSEC( TIMER_PERIOD_VALUE ) );
...
void led_timer_expiry_fn( struct k_timer *timer )
{
	...
	if( led_red_time_counter > LED_RED_PERIOD_VALUE )
	{
		led_red_time_counter = 0;
		gpio_pin_toggle_dt( &dt_led_red );
	}

	if( led_app_time_counter > LED_APP_PERIOD_VALUE )
	{
